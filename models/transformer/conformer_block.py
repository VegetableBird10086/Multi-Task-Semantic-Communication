...
class ConformerBlock(nn.Module):
    def __init__(self, hidden_size, intermediate_size, num_attention_heads, conv_depth_wise_kernel_size):
        super().__init__()

        self.ffn1 = FeedForward(hidden_size, intermediate_size)
        self.self_attn = SelfAttentionModule(hidden_size, num_attention_heads)
        self.conv_module = ConvolutionModule(hidden_size, conv_depth_wise_kernel_size)
...
class ConvolutionModule(nn.Module):
    def __init__(self, hidden_size, conv_depth_wise_kernel_size):
        super().__init__()
        ...
        self.depth_wise_conv = nn.Conv1d(
            hidden_size,
            hidden_size,
            conv_depth_wise_kernel_size,
            stride=1,
            padding=(conv_depth_wise_kernel_size - 1) // 2,
            groups=hidden_size,
            bias=False
        )
