...
from .shared_utils import cached_path
...
class VisualBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.projection = nn.ModuleDict({
            '2048': nn.Linear(2048, config.hidden_size),
            '1024': nn.Linear(1024, config.hidden_size),
            '512': nn.Linear(512, config.hidden_size),
            '256': nn.Linear(256, config.hidden_size)
        })
    ...
    def forward(self, text_feature, fusion_feat=None, visual_feats=None, visual_token_type_ids=None):
        embeddings = text_feature
        dim = visual_feats.shape[-1]
        visual_embeddings = self.projection[str(dim)](visual_feats)
        ...
        if not fusion_feat:
            ...
            embeddings = torch.cat((embeddings,visual_embeddings, fusion_feat),dim=1)
...
class VBBertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.pooler = BertPooler(config)
        self.apply(self.init_bert_weights)
    def forward(self, text_feature, token_type_ids=None, attention_mask=None,
                visual_feats=None, visual_token_type_ids=None,visual_attention_mask=None, fusion_feat=None, fusion_mask=None):
        
        if not fusion_feat:
            attention_mask = torch.cat((attention_mask,visual_attention_mask),dim=1)
        else:
            attention_mask = torch.cat((attention_mask,visual_attention_mask, fusion_mask),dim=1)
        ...
        embeddings = self.embeddings(text_feature, fusion_feat=fusion_feat,
...
class VBFeatureExtraction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_token_type_ids=None, visual_attention_mask=None, fusion_feat=None, fusion_mask=None):
        pooled_output = self.bert(input_ids,token_type_ids=token_type_ids,attention_mask=attention_mask,
                                  visual_feats=visual_feats,visual_token_type_ids=visual_token_type_ids,
                                  visual_attention_mask=visual_attention_mask, fusion_feat=None, fusion_mask=None)
        return pooled_output
