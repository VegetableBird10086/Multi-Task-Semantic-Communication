...
from .shared_block2 import VBFeatureExtraction as VBE
...
class SemanticEncoder(nn.Module):
    def __init__(
        ...
        self.shared_block = VBE.from_pretrained("bert-base-uncased")
    ...
    def forward(self, data):
        ...
        if task_type == 'fastvqa':
            ...
            visual_segment_ids = torch.ones(bs, data['image'].shape[1],dtype=torch.long).to(device)
            v_mask = torch.ones(bs, data['image'].shape[1],dtype=torch.long).to(device)
            data['memories'] = self.shared_block(input_ids = encoder_outputs, 
                                       token_type_ids =data['segment_ids'], 
                                       attention_mask =data['text_mask'],
                                       visual_feats = data['image'],
                                       visual_token_type_ids=visual_segment_ids,
            ...
            # data['memories'] = torch.cat((encoder_outputs, img_features), dim=1)
            # data['memories'] = self.text_proj(data['memories'])
            ...
            # data['image'] = img_features.repeat(1, 36, 1)
            ...
            # data['memories'] = self.text_proj(torch.cat((encoder_outputs, img_features), dim=1))
            ...
            #     head_mask=head_mask,
            # )  
            # data['memories'] = encoder_outputs[0]
            ...
            # img_features = img_features.flatten(1)
            # # img_features = img_features.unsqueeze(1)
            # data['memories'] = self.twitter(img_features)
